    ...
    public class Player : Actor
    {
        ...
        public void Die()
        {
            if (!isDead)
            {
                ...
                wait(1f, () =>
                {
                    this.Scene.GameManagement.CurrentGameplayStatus = UmbrellaToolsKit.GameManagement.GameplayStatus.DEATH;
                });
        ...
        public bool canRender {
            get => this.Scene.GameManagement.Values["freeze"] ||
                this.Scene.GameManagement.CurrentStatus == UmbrellaToolsKit.GameManagement.Status.MENU ||
                this.Scene.GameManagement.CurrentStatus == UmbrellaToolsKit.GameManagement.Status.LOADING;
        }
        ...
        {
            float angle = (float)Math.Atan2(position.Y, position.X);// / 180 * MathF.PI;
            this.Rotation = angle + ( 180.0f / (float)Math.PI * 180);
        ...
        {
            get => (float)Math.Cos((double)(timer * 0.0001f * _speed));
        }

        private float _getSinPosition
        {
            get => (float)Math.Sin((double)(timer * 0.0001f * _speed));
        }
        ...
        public void Jump()
        {
            ...
            if (this.JumpPressedBtn && this.JumpPressed && this.JumpPressedForce < 1)
            {
                ...
                float g = (2 * this.JumpForce) / ((float)Math.Pow(6f, 2f));
                float initJumpVelocity = (float)Math.Sqrt(8f * g * this.JumpForce);
